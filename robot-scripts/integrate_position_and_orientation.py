...
def get_up_marker_id_camera_marker_id(num_steps, cap, marker_dict, detector_params, cam_mat, dist_coef):
    ID_TO_Z_AXIS = {
    ...
    print(f"Up marker: {up_id}")
    print(f"Side marker: {side_id}")
    print(f"Num IDs seen: {len(ids_seen)}")
    return up_id, side_id    

def get_cube_x_position(num_steps,cap, marker_dict, detector_params, cam_mat, dist_coef, up_marker_id, camera_side_id):
...
def get_cube_x(CAMERA_NAME,CAMERA_NUMBER):
    '''
    A funtion that scans and estimates rubiks cube position and orientation from the base tag using Arcuo tags.
    ...
    up_marker_id, camera_side_id = get_up_marker_id_camera_marker_id(10, cap, marker_dict, detector_params, cam_mat, dist_coef)

    # get the cube x position
    cube_x = get_cube_x_position(100, cap, marker_dict, detector_params, cam_mat, dist_coef, up_marker_id, camera_side_id)
    print(f"Cube x position in meters: {cube_x}")

    cap.release()
    cv.destroyAllWindows()
    return cube_x, up_marker_id, camera_side_id
